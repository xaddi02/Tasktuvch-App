...
# 🔥 1. VARIABLES DE ENTORNO SECULARES 🔥
# (Ya no hay variables duplicadas abajo)
DB_URL = os.environ.get("DATABASE_URL")
NGROK_OLLAMA_URL = os.environ.get("NGROK_URL")

# --- 2. RUTAS DE RECURSOS (IMÁGENES, JUEGOS Y APRENDIZAJE) ---
...
@app.route('/juegosmc/<path:nombre_archivo>')
def servir_juegos(nombre_archivo):
    return send_from_directory(os.path.join(directorio_base, 'juegosmc'), nombre_archivo)

@app.route('/aprendizaje/<path:nombre_archivo>')
def servir_aprendizaje(nombre_archivo):
    return send_from_directory(os.path.join(directorio_base, 'aprendizaje'), nombre_archivo)
...
def guardar_datos():
    ...
    datos_app = json.dumps(peticion.get("datos"))
    try:
        conn = obtener_conexion()
        cursor = conn.cursor()
...
def chat():
    ...
    try:
        # Iniciamos el cliente de Ollama aquí adentro usando la variable limpia
